...
<main class="container-fluid bg-dark bg-opacity-25">
    ...
    <div class="pt-3 table-responsive-md">
        ...
        <table class="table table-bordered table-hover table-striped mt-3  rounded-top caption-top">
            ...
            <caption class="bg-dark text-light p-2 fs-5" style="border-radius: 30px 30px 0px 0px;">
                <div class="row align-items-center">
                    <span class="col-2" style="padding-left: 25px"><fmt:message key="show_users_tests.tests"/></span>
                    ...
                    <span class="col-1" style="padding-right: 25px">
                                ...
                                <select class="form-select" name="thisPaginationParameter" id="pagination">
                                    <option value="5"
                                            <c:if test="${requestScope.thisPaginationParameter==5}">selected</c:if> >5</option>
                                    <option value="10"
                                            <c:if test="${requestScope.thisPaginationParameter==10}">selected</c:if> >10</option>
                                    <option value="15"
                                            <c:if test="${requestScope.thisPaginationParameter==15}">selected</c:if> >15</option>
                                    <option value="20"
                                            <c:if test="${requestScope.thisPaginationParameter==20}">selected</c:if> >20</option>
                                </select>
            ...
            <tr>
                ...
                <td class="text-center">
                    <button class="btn btn-danger" type="button" data-bs-toggle="modal"
                            data-bs-target="#deleteModal${i}"><fmt:message key="button.delete"/>
                    </button>
                </td>
                <!-- Delete Modal -->
                <div class="modal fade" id="deleteModal${i}" tabindex="-1" aria-labelledby="exampleModalLabel"
                     aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel"><fmt:message key="modal.delete.passed.test"/></h5>
                            ...
                            <form method="post" action="/web-application/testing/admin/deletePassedTest">
                                <div class="modal-body">
                                    ...
                                    <input required id="secretPassword" type="password" name="secretPassword"
                                           class="form-control">
                                    <input hidden type="text" name="thisPageNumber"
                                           value="${requestScope.thisPageNumber}">
                                    <input hidden type="text" name="thisPaginationParameter"
                                           value="${requestScope.thisPaginationParameter}">
                        ...
                        <c:if test="${requestScope.thisPageNumber!=i}">
                            <li class="page-item ">
                                <button class="page-link1 bg-dark border-dark text-white" type="submit"
                                        name="thisPageNumber" value="${i}">${i}</button>
                            </li>
                        </c:if>
        ...
        <c:if test="${empty testList}">
            <div class="w-50 m-auto bg-dark  mt-3 d-flex align-items-center"
                 style="border-radius: 30px 30px 30px 30px;box-shadow: 0px 0px 50px 1px rgba(0,0,0,0.5); margin-top: 10px; height: 5%">
                <span class="text-white ms-3  "><fmt:message key="show_users_tests.not_passed"/></span>
            </div>
        </c:if>
